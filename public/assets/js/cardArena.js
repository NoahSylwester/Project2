...
// end game flag
  ...
  // handle different length numbers for atk and def and cost, to be used in drawing text
    ...
    if (radians !== undefined && this.grabbed === false) {
      ...
      c.rotate(radians);
      c.drawImage(this.sprite.art, -this.sprite.width/2, -this.sprite.height/2 + (Math.abs(radians)+1)**3 * 12, this.sprite.width, this.sprite.width * artRatio);
    ...
    // highlight selected card
      ...
      c.drawImage(this.sprite.art, this.x, this.y, this.sprite.width, this.sprite.width * artRatio);
  ...
  // necessary to re-add functions and img to cards after they go through the server
    ...
    // adds canvas functions
      ...
      array[i].cardSprite.sprite.art = document.querySelector(`#${array[i].cardSprite.sprite.artPath}`);
    }
  };
  ...
  // card constructor
  function Card(art, img, atk, def, ability, cost = 0, name = "Monster") {
  // everything is within this cardSprite object
    ...
    // initialize easy-to-verify id
      ...
      sprite: {
        artPath: art.slice(0, art.length - 4), // used in parseCards function
        art: document.querySelector(`#${art.slice(0, art.length - 4)}`),
        img: img,
 ...
 * Taken from a stackoverflow answer by user 'Laurens Holst'
 */
  ...
  function animate() {
    ...
    if (zoomedCard !== undefined) {
      ...
      c.drawImage(zoomedCard.cardSprite.sprite.art, x, y, zoomWidth, zoomWidth * artRatio);
